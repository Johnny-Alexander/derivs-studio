...
// Sample a Poisson(λ) count from a uniform u on (0,1) via inverse-CDF.
// O(k); fine in the small-λ·dt regime that v4's Merton steps live in (typically
// λ·dt < 0.05). Caps at k=50 as a guardrail.
export function poissonFromUniform(u, lambda) {
  if (lambda <= 0 || u <= 0) return 0;
  let k = 0;
